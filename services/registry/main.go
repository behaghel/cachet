...
func main() {
	...
	port := config.ResolvePort("PORT", cfg.Services.Registry.Port)
	...
	log.Info().
		...
		Str("host", cfg.Services.Registry.Host).
		Int("config_port", cfg.Services.Registry.Port).
		Str("effective_port", port).
		Str("public_url", cfg.Services.Registry.PublicURL).
